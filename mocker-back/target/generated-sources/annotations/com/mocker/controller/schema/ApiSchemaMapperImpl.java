...
@Component
public class ApiSchemaMapperImpl implements ApiSchemaMapper {

    @Autowired
    private ApiProjectMapper apiProjectMapper;

    @Override
    public Schema map(SchemaDto schemaDto) {
        if ( schemaDto == null ) {
            return null;
        }
        ...
        schema.project( apiProjectMapper.map( schemaDto.getProject() ) );
    ...
    @Override
    public List<SchemaDto> map(List<Schema> schemas) {
        if ( schemas == null ) {
            return null;
        }

        List<SchemaDto> list = new ArrayList<SchemaDto>( schemas.size() );
    ...
    @Override
    public SchemaDto map(Schema schema) {
        if ( schema == null ) {
            return null;
        }
        ...
        schemaDto.setProject( apiProjectMapper.map( schema.getProject() ) );
        schemaDto.setCreatedDate( schema.getCreatedDate() );
        if ( schema.getCreatedBy() != null ) {
            schemaDto.setCreatedBy( UUID.fromString( schema.getCreatedBy() ) );
        }
        if ( schema.getCreatedByGroup() != null ) {
            schemaDto.setCreatedByGroup( UUID.fromString( schema.getCreatedByGroup() ) );
        }
        schemaDto.setModifiedDate( schema.getModifiedDate() );
        if ( schema.getModifiedBy() != null ) {
            schemaDto.setModifiedBy( UUID.fromString( schema.getModifiedBy() ) );
        }
        if ( schema.getModifiedByGroup() != null ) {
            schemaDto.setModifiedByGroup( UUID.fromString( schema.getModifiedByGroup() ) );
        }
    ...
    protected Table tableDtoToTable(TableDto tableDto) {
        ...
        table.schema( map( tableDto.getSchema() ) );
